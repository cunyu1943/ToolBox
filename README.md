...
> 全部工具的核心逻辑均为 `utils/` 纯函数（`pnpm test` 共 482 例通过），页面组件直接调用，测试与线上代码同源。
