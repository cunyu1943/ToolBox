  ...
  <div class="space-y-4">
    <GlassCard custom-class="p-5 space-y-3">
      ...
      <div v-if="file" class="space-y-3">
        ...
        <textarea :value="dataUrl" readonly rows="5" class="w-full rounded-lg bg-slate-50 p-3 font-mono text-xs dark:bg-white/5" />
      </div>
    ...
    <GlassCard custom-class="p-5 space-y-3">
      ...
      <UTextarea v-model="pasted" :rows="4" placeholder="粘贴 data:image/...;base64,... 完整前缀" autoresize :maxrows="10" class="w-full font-mono text-xs" />
      <p v-if="pasted.trim() && !parsed" class="text-sm text-red-500">不是合法的 dataURL（需以 data: 开头且含 ;base64,）</p>
      <div v-if="parsed" class="space-y-2">
        <img :src="pasted.trim()" alt="还原预览" class="max-h-60 max-w-full rounded-lg border border-slate-200/60 dark:border-white/10">
...
const pasted = ref('')
const parsed = computed(() => (pasted.value.trim() ? parseDataUrl(pasted.value) : null))
